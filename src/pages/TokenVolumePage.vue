<template>
  <div class="token-volume-page">
    <h1>Token Volume Testing Page</h1>
    <p>
      Use this page to test fetching token volume data. Enter a token address and
    ...
    <div class="form-section">
      ...
      <div class="options">
        ...
        <button @click="fetchVolumeData" :disabled="!tokenAddress || isLoading">
...
export default {
  setup() {
      ...
      if (isLooping.value) {
        ...
        if (!stopLoop) {
          stopLoop = runWithInterval(async () => {
            ...
            try {
              logToConsole(`Fetching volume data for token: ${tokenAddress.value}`);
              await fetchAndInsertTokenVolume(tokenAddress.value.trim());
              logToConsole('Volume data fetched and saved successfully.');
            } catch (error) {
    ...
    return { tokenAddress, isLoading, isLooping, consoleLog, fetchVolumeData };
  },
};
</script>
